...
class Template
{
    ...
    public static function render($template, $vars = array(), $baseDir=null)
    {
    ...
    private static function highlight($template, $vars = array(), $baseDir = "")
    {
        ...
        echo '<div class="code__title">' . icon('console') . ' Code "templates/' . $template . '"</div>';
        echo '<code class="code__lines">', implode(range(1, count(file($templatePath))), '<br />'), '</code>';
        highlight_file($templatePath);
    }
